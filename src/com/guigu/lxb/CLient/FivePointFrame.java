...
class FivePointFrame extends JFrame
{
	...
	public String NEW_LINE=System.getProperty("line.separator");   //相当于换行
	...
	public FivePointFrame()
	{
		...
		myInfoPane.add(new JLabel(new ImageIcon("2.gif")));
		myInfoPane.add(info);
		
		JPanel peerInfoPane=new JPanel();
		peerInfoPane.setLayout(new GridLayout(0,1));		
		peerInfoPane.add(new JLabel(new ImageIcon("3.gif")));
		...
		createBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent arg0) {
				...
				notify.append("游戏创建成功"+System.getProperty("line.separator"));    //换行
		...
		startBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent arg0) {
				...
				startBtn.setEnabled(false);				//启用禁用按钮
				addBtn.setEnabled(false);
				cancelBtn.setEnabled(false);
				createBtn.setEnabled(false);
		...
		try {		
			serverIP=InetAddress.getByName("192.168.8.233");       				  //服务器IP
	...
	public void flushPeerInfo()
	{
		...
		//此处可考虑加入数据库，对数据库语句进行更新（运用update语句）
		SwingUtilities.invokeLater(new Runnable(){
			public void run() {
